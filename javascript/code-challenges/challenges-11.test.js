...
const count = (target, input) => {
  return input.reduce((count, array) => {
    return count + array.filter(element => element === target).length;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((sum, array) => {
    return sum + array.reduce((acc, num) => acc + num, 0);
  }, 0);
};
...
let findShortest = (data) => {
  return.data.reduce((shortestSoFar, currChar) => Number(shortestSoFar.height) < Number (currChar.height) ? shortestSoFar : currChar).name;
};
